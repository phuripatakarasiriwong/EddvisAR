...
<div class="heading">Heading: <span id="headingReading">-</span>&deg;</div>
...
<script type="text/javascript">
  ...
  window.addEventListener('load', function() {
    ...
    <?php
            ...
            $sql = "SELECT * FROM arobject where place_name='Thailand';";
              $result = $conn->query($sql);
              if ($result->num_rows > 0) {
                // output data of each row
                  while($row = $result->fetch_assoc()) {

                    echo "AR.push('".$row['ar_id']."');";
                    echo "AR.push(".$row['ar_x'].");";
                    echo "AR.push(".$row['ar_y'].");";
                    echo "AR.push(".$row['ar_z'].");";
                    echo "AR.push(".$row['ar_file'].");";
                    if($row['ar_x']==200)echo "ARint.push(0);";
                    if($row['ar_x']== -200)echo "ARint.push(2);";
                    if($row['ar_x']== 0 && $row['ar_z'] == 200)echo "ARint.puash(1);";
                    if($row['ar_x']== 0 && $row['ar_z'] == -200)echo "ARint.puash(3);";
   ...
   function Resize(){       
              ...
              var c = awe.projections.view(AR[i]);
                awe.projections.update({ // rotate clicked object by 180 degrees around x and y axes over 10 seconds
                  data:{
                    ...
                    scale: { x: c.scale.x+(scaling * size[ARint[i]]), y: c.scale.y+(scaling * size[ARint[i]]), z: c.scale.z+(scaling * size[ARint[i]])},
                  },
                  where:{ id:AR[i]}
                });
              }
    ...
    function Startobject(){
              ...
              for(i = 0;i< AR.length;i++){
              var a = awe.projections.view(AR[i]);
                awe.projections.update({ // rotate clicked object by 180 degrees around x and y axes over 10 secondsx
                  data:{
